...
class Command(BaseCommand):
    ...
    def handle(self, term=None, school='orange', **options):
...
def run_csv_out(term, school):
        ...
        csv_header = csv.reader(header_file,
                                delimiter=',',
                                quotechar='"'
        )
        ...
        section_set = model2.objects.distinct('section_number')
        course_set = model2.objects.distinct('course_number')
        ...
        for a_section in section_set:
            for a_course in course_set:
                ...
                my_expression = ''
                new_set = model2.objects.all().filter(
                ...
                if new_set:
                    if new_set.count() <= 3:
                        ...
                        if new_set.filter(period=dict_periods[0].period).count() == 3:
                            total_expression = '%s(%s,%s,%s)' % (
                                new_set[0].period,
                                new_set[0].day,
                                new_set[1].day,
                                new_set[2].day
                            )
                            total_expression += '%s(%s)' % (
                                new_set[3].period, new_set[3].day)
                ...
                if new_set:
                    tmp_item = new_set[0]
                    my_csv_row = [
                        tmp_item.course_number, tmp_item.day, tmp_item.period, tmp_item.course_name, tmp_item.section_number, tmp_item.termid,
                        ...
                        tmp_item.excludefromstoredgrades, tmp_item.maxenrollment
                    ]
